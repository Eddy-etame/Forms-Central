# Microservice de Formulaires Centralisé (Formdock)
...
### 🎓 Tutoriel Complet pour Débutants (Recommandé avec Protection Anti-Spam)

Pour intégrer ce formulaire sur votre site web de manière ultra-sécurisée (avec la Preuve de Travail pour bloquer les robots), suivez ce guide pas à pas.

**Étape 1 : Le code HTML**
Ajoutez ce formulaire où vous le souhaitez sur votre page web. Notez l'ID `mon-formulaire` et l'élément `message-retour` qui servira à afficher les erreurs ou le succès.

```html
<form id="mon-formulaire">
  <!-- Honeypot : Piège caché pour les robots (ne pas modifier) -->
  <input type="text" name="_gotcha" style="display:none !important" tabindex="-1" autocomplete="off" />
  ...
  <button type="submit" id="bouton-envoi">Envoyer le message</button>
...
**Étape 2 : Le script JavaScript (Logique d'envoi et Anti-Spam)**
Placez ce script juste avant la fermeture de votre balise `</body>`. **N'oubliez pas de remplacer `VOTRE_UUID_ICI` et `VOTRE_DOMAINE_API_ICI` par vos vraies valeurs !**

```html
<script>
// 1. Remplacez ces valeurs par les vôtres !
const API_BASE_URL = "https://VOTRE_DOMAINE_API_ICI"; // Ex: https://mon-api.vercel.app
const FORM_ID = "VOTRE_UUID_ICI"; // L'ID généré dans votre tableau de bord

// Fonction pour hacher (sécurité PoW)
...
document.getElementById('mon-formulaire').addEventListener('submit', async function(e) {
  ...
  const form = e.target;
  const btn = document.getElementById('bouton-envoi');
  const messageDiv = document.getElementById('message-retour');
  
  // Désactiver le bouton pendant l'envoi
  btn.disabled = true;
  btn.innerText = "Traitement sécurisé en cours...";
  messageDiv.innerText = "";
  messageDiv.style.color = "black";

  try {
    // ÉTAPE A : Récupérer toutes les données du formulaire
    const formData = new FormData(form);
    const dataPayload = Object.fromEntries(formData.entries());
    // Ajouter la langue pour l'auto-répondeur (optionnel)
    dataPayload._lang = "fr";

    // ÉTAPE B : Résoudre le défi cryptographique (Bloque les robots spammers)
    const challengeRes = await fetch(`${API_BASE_URL}/api/challenge`);
    ...
    // Boucle de calcul (prend quelques millisecondes)
    while (true) {
    ...
    // Ajouter la preuve de travail aux données
    dataPayload.pow_challenge = challenge;
    dataPayload.pow_timestamp = timestamp;
    dataPayload.pow_nonce = nonce.toString();

    // ÉTAPE C : Envoyer le message au serveur
    const response = await fetch(`${API_BASE_URL}/api/submit/${FORM_ID}`, {
      method: "POST",
    ...
    // ÉTAPE D : Gérer la réponse
    if (response.ok) {
      // Succès !
      messageDiv.style.color = "green";
      messageDiv.innerText = "Votre message a été envoyé avec succès !";
      form.reset(); // Vider le formulaire
    } else {
      // Erreur serveur ou validation
      messageDiv.style.color = "red";
      messageDiv.innerText = "Erreur : " + (result.error || "Impossible d'envoyer le message.");
    }
...
Grâce à ce code, vous avez désormais un formulaire qui intercepte automatiquement les robots, communique avec votre base de données centrale, et donne un retour visuel clair à vos utilisateurs !
