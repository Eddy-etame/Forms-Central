...
const STATUS = {
  healthy:  { label: 'Healthy',   ring: 'border-emerald-200 bg-emerald-50', dot: 'bg-emerald-500', text: 'text-emerald-700', Icon: CheckCircle2 },
  degraded: { label: 'Recovering', ring: 'border-amber-200 bg-amber-50',    dot: 'bg-amber-500',   text: 'text-amber-700',   Icon: Activity },
  failing:  { label: 'Failing',    ring: 'border-red-200 bg-red-50',        dot: 'bg-red-500',     text: 'text-red-700',     Icon: AlertTriangle },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Email health</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: 'Sending accounts', value: data ? `${data.activeCount} / ${data.totalConfigured}` : '—', hint: 'active / configured', Icon: Mail, tone: data && data.activeCount === 0 ? 'text-red-600' : 'text-emerald-600', tile: data && data.activeCount === 0 ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600' },
          { label: 'Outages (24h)', value: data ? String(data.outages24h) : '—', hint: 'all-accounts-failed events', Icon: ServerCrash, tone: data && data.outages24h > 0 ? 'text-red-600' : 'text-slate-500', tile: data && data.outages24h > 0 ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-500' },
          { label: 'Last outage', value: data ? timeAgo(data.lastOutageAt) : '—', hint: data?.lastOutageAt ? 'most recent total failure' : 'none recorded', Icon: XCircle, tone: 'text-slate-500', tile: 'bg-slate-50 text-slate-500' },
        ].map((c) => (
      ...
      {loading && !data ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500 shadow-sm">
          ...
          No sending accounts configured. Add <code className="rounded bg-slate-100 px-1 py-0.5 text-xs">SMTP_*</code> credentials to your environment.
        </div>
      ) : (
                ...
                <div className={`rounded-2xl border ${s.ring} p-5 shadow-sm`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-slate-900">{acc.label.replace('account-', 'Account ')}</span>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-slate-200/60 pt-3 text-xs">
                    <span className="text-slate-500">
                      Failures (24h): <span className="font-bold text-slate-700 tabular-nums">{acc.failures24h}</span>
                    </span>
      ...
      <p className="text-xs text-slate-400">
        Disable a suspended account cleanly by setting <code className="rounded bg-slate-100 px-1 py-0.5">SMTP_&lt;n&gt;_DISABLED=true</code> in your
