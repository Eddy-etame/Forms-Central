...
function buildHandler(auth: ApiKeyAuth) {
  return createMcpHandler(
    (server) => {
      server.tool(
        'list_forms',
        'List all forms in this Inlet account with their IDs and status.',
        {},
        async () => {
    ...
    {
      serverInfo: { name: 'inlet', version: '1.0.0' },
    },
    {
      basePath: '/api',
      maxDuration: 60,
    ...
    return Response.json(
      {
        ...
        error: {
          code: -32001,
          message: `Unauthorized: pass a Inlet API key as "Authorization: Bearer kef_...". Create one in your dashboard (${SERVICE_ORIGIN}/client/dashboard/developer).`,
        },
        id: null,
      },
      { status: 401 }
