    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Give each client a private, branded portal to view only their own leads.</p>
        </div>
        {enabled && (
          <button
      ...
      {!enabled && !loading && (
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-center">
          ...
          <p className="font-semibold text-slate-900">Client portals start on the Solo plan</p>
          <p className="mx-auto mt-1 max-w-md text-sm text-slate-500">Create white-label portals so each client logs in and sees only their leads.</p>
      ...
      {enabled && (
        <div className="flex items-center gap-2 text-xs text-slate-400">
          <Link2 className="h-3.5 w-3.5" /> Portal login: <code className="text-slate-600">{portalUrl}</code>
      ...
      {loading ? (
        ...
        enabled && users.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-10 text-center">
            ...
            <p className="font-semibold text-slate-900">No end-clients yet</p>
            <p className="mx-auto mt-1 max-w-sm text-sm text-slate-500">Create one, then assign the forms whose leads they should see.</p>
          </div>
        ) : (
