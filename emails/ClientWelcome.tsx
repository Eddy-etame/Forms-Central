...
export const ClientWelcomeEmail = ({
  ...
  return (
    <Html>
      <Head />
      <Preview>{isReset ? (isEn ? 'Reset your password' : 'Réinitialisation de votre mot de passe') : (isEn ? 'Welcome to your Client Portal' : 'Bienvenue sur votre Espace Client')}</Preview>
            ...
            {!isReset ? (
              <>
                <Text style={{ color: '#555', fontSize: '16px', lineHeight: '24px' }}>
                  {isEn
                    ? 'Your client portal has been created successfully. You can now sign in to manage your forms and view your leads.'
                ...
                <Section style={{ backgroundColor: '#f9fbfd', padding: '16px', borderRadius: '8px', marginTop: '16px', border: '1px solid #e6ebf1' }}>
                  <Text style={{ margin: '8px 0', color: '#333', fontSize: '15px' }}>
                    <strong>{isEn ? 'Login email:' : 'Email de connexion :'}</strong> {clientEmail}
                  </Text>
                  <Text style={{ margin: '8px 0', color: '#333', fontSize: '15px' }}>
              ...
              <>
                <Text style={{ color: '#555', fontSize: '16px', lineHeight: '24px' }}>
                  {isEn
                    ? 'You requested a password reset. Your new temporary password has been generated:'
